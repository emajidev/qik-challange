...
import { IRidersDocument } from 'src/core';
import axios from 'axios';

@Injectable()
export class ridersCommand {
  constructor(
    @InjectModel('riders') private readonly ridersModel: Model<IRidersDocument>,
  ) { }

  @Command({
    command: 'seed:riders',
    describe: 'Seed riders collection with locations at 1km, 3km and 6km from current position',
  })
  async seed() {
    try {
      const count = await this.ridersModel.countDocuments();
      if (count > 0) {
        console.log('riders collection already has data. Skipping seeding.');
        return;
      }

      const currentLocation = await this.getCurrentLocation();
      console.log('Seeding riders around your current location:', currentLocation);

      await this.importFromJson(currentLocation);
      console.log('riders seeding completed successfully!');
    } catch (error) {
  ...
  private async importFromJson(currentLocation: { longitude: number, latitude: number }) {
    console.log('Starting riders import with variable distances...');

    const items = require('./riders.json');
    ...
    const result = await this.ridersModel.insertMany(ridersToInsert);

    console.log(`Inserted ${result.length} riders at variable distances from your location:`);
    console.log(`- ${groups[0].count} riders at ~1km`);
    console.log(`- ${groups[1].count} riders at ~3km`);
    console.log(`- ${groups[2].count} riders at ~6km`);
